...
const SAVE_KEY = 'wildlands_survival_v5.7.5'; 
const VERSION = 'v5.7.5 "Modern Survivor"';
const SPAWN_X = 160; 
const CENTER_Z = 120;
...
const App: React.FC = () => {
  ...
  const addNotification = useCallback((text: string) => {
    ...
    setNotifications(prev => {
      // Mesaj yığılmasını önle, max 5 mesaj
      const next = [...prev, { id, text }];
      if (next.length > 5) return next.slice(next.length - 5);
      return next;
    });
    // Mesajları 3 saniye sonra sil
    setTimeout(() => { 
  ...
  const handleCraft = useCallback((type: string) => {
    setGameState(prev => {
      ...
      let success = false; let cost: Record<string, number> = {}; let resultName = ""; let fuelCost = 0;
      ...
      else if (type === 'cook_fruit' && getCount('Fruit') >= 1 && isNearFire) { success = true; cost = { 'Fruit': 1 }; resultName = "Cooked Fruit"; fuelCost = 30; }
      ...
      if (success) {
        ...
        if (type === 'campfire') {
          ...
          const spawnDist = 4.5; const spawnX = playerInfoRef.current.x + playerInfoRef.current.dirX * spawnDist; const spawnZ = playerInfoRef.current.z + playerInfoRef.current.dirZ * spawnDist;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Ateş başındayken Pişirme Kısa yolları (F ve G)
  ...
  }, [view, isCraftingOpen, gameState.inventory, interaction.type, handleUseItem, handleCraftMenuToggle, handleCraft]);
  ...
  const handleCollect = useCallback((type: string) => {
    setGameState(prev => {
      ...
      if (type === 'Sleep') {
        playSFX(SFX_URLS.ui_click); addNotification(t.sleep + " (+5h)");
        let nextTime = prev.time + 500; let nextDay = prev.day;
        if (nextTime >= 2400) { nextTime -= 2400; nextDay += 1; }
        return { ...prev, time: nextTime, day: nextDay, stats: { ...prev.stats, health: 100, energy: 100, temperature: 25 } };
      }
      if (type === 'Water') {
  ...
  return (
    <div className="w-screen h-screen bg-slate-950 text-white font-sans overflow-hidden select-none">
      ...
      {view !== 'loading' && (
        <>
          <GameScene 
            ref={sceneRef} time={gameState.time} campfires={gameState.campfires} shelters={gameState.shelters} isLocked={isLocked} isMobile={isMobile}
