...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeys = (e: KeyboardEvent) => {
      ...
      if (code === 'KeyB') {
        const item = gameState.inventory.find(i => i.name === 'Bow');
  ...
  return (
    <div className="w-screen h-screen bg-slate-950 text-white font-sans overflow-hidden" onClick={() => view === 'game' && !isMobile && sceneRef.current?.requestLock()}>
      ...
      <UIOverlay 
        ...
        isMobile={isMobile}
        onCook={() => {}} // Placeholder as handleCook logic is inside App in older version, now mapped to interactions
      ...
      {view === 'menu' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-xl">
          <h1 className="text-8xl font-black mb-12 italic tracking-tighter text-indigo-500">WILD LANDS</h1>
          <button onClick={() => setView('game')} className="px-12 py-6 bg-white text-black font-black rounded-2xl text-2xl hover:scale-105 transition-transform">OYUNA BAŞLA</button>
        </div>
      )}
      {isGameOver && (
        <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-black text-red-600">
          <h1 className="text-9xl font-black mb-8">ÖLDÜN</h1>
          <button onClick={() => window.location.reload()} className="px-8 py-4 bg-red-600 text-white rounded-xl font-bold">TEKRAR DENE</button>
        </div>
      )}
    </div>
  );
