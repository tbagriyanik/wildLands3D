...
const dist2D = (x1: number, z1: number, x2: number, z2: number) => {
...
const GameScene = forwardRef<GameSceneHandle, GameSceneProps>(({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 3500; i++) {
      ...
      const dToStart = dist2D(x, z, 120, 120);
      if (Math.sqrt(x * x + z * z) > 55 && dToStart > 12) {
      ...
      if ((isLocked || isMobile) && !isCraftingOpen) {
        ...
        if (Math.abs(moveX) > 0.01 || Math.abs(moveY) > 0.01) {
          ...
          for (const o of worldObjectsRef.current) {
            if (o.userData.isObstacle) {
              const d = dist2D(nextPos.x, nextPos.z, o.position.x, o.position.z);
              if (d < (0.6 + (o.userData.radius || 0.8))) { collided = true; break; }
            }
          }
